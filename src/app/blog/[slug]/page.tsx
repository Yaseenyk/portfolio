...
// Internal linking: rank every other post by tag/keyword overlap with the
// current one (computed at build — no client cost) and surface the top 3.
function relatedPosts(current: ResolvedPost): RelatedPost[] {
  const tsxSlugs = new Set(getAllSlugs());
  const pool = [
    ...getAllPosts(),
    ...getAllMdxMeta().filter((m) => !tsxSlugs.has(m.slug)),
  ];
  const currentTerms = new Set(
    [...current.tags, ...current.keywords].map((t) => t.toLowerCase()),
  );
  ...
  return pool
    ...
    .map((p) => ({
      post: p,
      score: [...p.tags, ...p.keywords].filter((t) =>
        currentTerms.has(t.toLowerCase()),
      ).length,
    }))
    .filter((r) => r.score > 0)
...
// Covers generated by scripts/generate-og-images.py land in public/og/ and
// are picked up here automatically — no per-post ogImage wiring needed.
function resolveOgImage(slug: string, explicit?: string): string | undefined {
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-12">
      ...
      {/* Internal linking: related articles by tag/keyword overlap */}
      {related.length > 0 && (
          ...
          <ul className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {related.map((r) => (
              <li key={r.slug}>
                <Link
                  href={`/blog/${r.slug}`}
                  className="group flex h-full flex-col rounded-2xl border border-zinc-800/60 bg-zinc-950/60 p-5 transition-colors hover:border-cyan/50"
                >
                  <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-500">
                    {r.readingMinutes} min read
                  </span>
                  <span className="mt-2 text-sm font-semibold leading-snug text-zinc-100 transition-colors group-hover:text-cyan">
                    {r.title}
                  </span>
